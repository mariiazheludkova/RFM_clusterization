...
if uploaded_file is not None:
   ...
   # ============================================
    ...
    if st.session_state.run_clicked and 'labels' in st.session_state:
        ...
        st.markdown("""
        ### Приклади інтерпретації кластерів товарів:
        - **Кластер з високим MonetaryValue та Frequency** → Товари-бестселери (генерують основний виторг, стабільний попит)
        - **Кластер з високим Recency** → Товари, які «зависли» на складі (їх давно ніхто не купував)
        - **Кластер з низьким Recency та високим Quantity** → Трендові або сезонні масові товари, що активно продаються прямо зараз
        - **Noise points у DBSCAN (-1)** → Унікальні або аномально дорогі/специфічні товари
        """)
else:
    st.info("⬅️ Завантажте датасет для початку роботи")
